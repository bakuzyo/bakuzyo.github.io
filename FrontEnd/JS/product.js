...
adress('http://localhost:3000/api/cameras' + '/' + elementId)
    ...
    prix.innerHTML = (element.price / 100).toFixed(2) +" €";         // Prix du produit
    nom.innerHTML = element.name;
    description.innerHTML = "<strong>Description : </strong>" + "<br> <br>" + element.description + element.description + "</br>" + element.description;
    ...
    btnPanier.addEventListener('click', function (ajout) {
        ...
        const panierAjout = JSON.parse(localStorage.getItem('panier')); // Récupération et affichage des éléments depuis le localStorage
